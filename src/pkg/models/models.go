...
type Config struct {
	...
	MicrophoneDeviceIndex     int               `json:"microphone_device_index"`
	AskForMicrophoneSelection bool              `json:"ask_for_microphone_selection"`


	DockerMode bool `json:"docker_mode"` // true enables Docker-compatible audio handling
	

	MCPServerEnabled bool `json:"mcp_server_enabled"` // true enables MCP server
	MCPServerPort    int  `json:"mcp_server_port"`    // MCP server port
	

	LLMCallInterval Duration `json:"llm_call_interval"` // Interval between LLM calls for actions (e.g., 5 minutes)
}
