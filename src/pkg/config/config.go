...
var envVarInQuotesRegex = regexp.MustCompile(`"\$\{([^:}]+)(?::([^}]*))?\}"`)
...
func processEnvVars(content string) string {
	// First, handle environment variables inside quotes (for boolean and numeric values)
	result := envVarInQuotesRegex.ReplaceAllStringFunc(content, func(match string) string {
		matches := envVarInQuotesRegex.FindStringSubmatch(match)
		if len(matches) < 2 {
			return match
		}
		...
		if value == "true" || value == "false" {
			return value // Return without quotes for boolean values
		}
		// Check if value is a number (for integer fields)
		if isNumeric(value) {
			return value // Return without quotes for numeric values
		}
		if value == "" {
			return `""`
		}
		return `"` + value + `"` // Return with quotes for string values
	})

	// Then handle remaining environment variables (not inside quotes)
...
func LoadProfiles(filename string) ([]models.Config, error) {
	...
	processedContent := processEnvVars(string(content))

	var profiles []models.Config
	if err := json.Unmarshal([]byte(processedContent), &profiles); err != nil {
...
func SaveProfiles(configs []models.Config) error {
	data, err := json.MarshalIndent(configs, "", "  ")
	if err != nil {
		return err
	}
	return os.WriteFile(filepath.Join(GetUserConfigPath(), PROFILES_FILE_NAME), data, 0644)
}
...
func DefaultProfile() models.Config {
	return models.Config{
		...
		IdleTimeout:               models.Duration{Duration: 3 * time.Minute},
		...
		SessionTimeout:            models.Duration{Duration: 1 * time.Hour},
		MicrophoneDeviceIndex:     -1, // -1 means not set, will ask user
		AskForMicrophoneSelection: true,
		DockerMode:                false,
		MCPServerEnabled:          true,
		MCPServerPort:             8089,
		LLMCallInterval:           models.Duration{Duration: 5 * time.Minute},
...
func LoadConfig(profileName string, debug bool) (*models.Config, error) {
	...
	if debug {
		...
		cfg.DatabaseFile = filepath.Join(logDir, "focus_helper_debug.db")
		cfg.LogFile = filepath.Join(logDir, "focus_helper_debug.log")

		log.Printf("Debug log: %s DB file: %s", cfg.LogFile, cfg.DatabaseFile)
