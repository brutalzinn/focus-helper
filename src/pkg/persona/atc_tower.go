...
func (a *ATCPersona) ProcessAudio(text string) error {
	...
	backgroundAudioPath := config.GetAssetPath("radio_static.wav")
	originalFilePath := filepath.Join(config.GetUserConfigPath(), config.TEMP_AUDIO_DIR, fmt.Sprintf("%d_%s.wav", timestamp, a.GetName()))
	tempFiltered := filepath.Join(config.GetUserConfigPath(), config.TEMP_AUDIO_DIR, fmt.Sprintf("%d_%s_temp_filter.wav", timestamp, a.GetName()))
	finalFilePath := filepath.Join(config.GetUserConfigPath(), config.TEMP_AUDIO_DIR, fmt.Sprintf("%d_%s_final.wav", timestamp, a.GetName()))
	defer func() {
